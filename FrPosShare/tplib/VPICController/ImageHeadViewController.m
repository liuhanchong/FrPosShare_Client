...
@interface ImageHeadViewController() <UINavigationControllerDelegate, UIImagePickerControllerDelegate, VPImageCropperDelegate>

@end

@implementation ImageHeadViewController
...
- (void) addImageHeadView
{
    [self.viewController.view addSubview:self.portraitImageView];
}

- (void)loadPortrait {
    [self addImageHeadView];
    self.portraitImageView.image = [UIImage imageNamed:@"ehead"];
    
}

- (void)editPortrait {
    UIAlertController* choiceSheet = [UIAlertController
    ...
    UIAlertAction* albumButton = [UIAlertAction
                              actionWithTitle:@"相册"
                              style:UIAlertActionStyleDefault
                              handler:^(UIAlertAction * action)
                              {
    ...
    UIAlertAction* selfButton = [UIAlertAction
                                  actionWithTitle:@"默认"
                                  style:UIAlertActionStyleDefault
                                  handler:^(UIAlertAction * action)
                                  {
                                      self.portraitImageView.image = [UIImage imageNamed:@"dehead"];
                                  }];
    
    [choiceSheet addAction:albumButton];
    [choiceSheet addAction:cameraButton];
    [choiceSheet addAction:selfButton];
    [choiceSheet addAction:cancelButton];
    
    [self.viewController presentViewController:choiceSheet animated:YES completion:nil];
}
...
- (UIImageView *)portraitImageView {
    if (!_portraitImageView) {
        ...
        CGFloat y = (self.viewController.view.frame.size.height - h) / 4;
